<?php
  ...
  ***********************/
    private $debug = true;   // variable activant les messages d'informations de debug
    private $serveur = "";    // adresse ip ou nom du serveur hébergeant le LDAP - NE PAS INITIALISER ICI SA VALEUR PAR DEFAUT
  ...
  ***********************/
    public function __construct() {
		...
		$nb_args = func_num_args(); // récupération du nombre de paramètres à l'appel de la fonction.
		if ($nb_args == 0) {
			$this->serveur = "10.108.80.3"; // adresse mémorisée (ou nom) du serveur dans la donnée membre ; correspond à la valeur par défaut
		}
		else  {
			$this->serveur = func_get_arg(0); // récupération du 1er paramètre (la numérotation commence par 0)
		}
